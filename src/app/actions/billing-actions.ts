...
/**
 * @fileoverview Server Actions antigas (compatibilidade)
 * 
 * Este arquivo contém server actions que precisam ser chamadas de Client Components.
 * Separado para evitar conflitos com imports de supabase-server.
 */
...
export async function setupTrialSubscription(userId: string) {
  const { setupTrial } = await import('./billing/plans')
...
export async function getBlockedSuppliers(userId?: string): Promise<{
  ...
  const { getEffectiveSubscription } = await import('./billing/plans')
...
export async function getInvoicesAction() {
  const { createClient } = await import('@/lib/supabase-server')
  const { AsaasService } = await import('@/lib/clients/asaas')
